...
#Load data ####
{
abund_seq <- read.csv("ASV_abundance_Table_ITS.csv",sep=',',header = FALSE)
seqs<-abund_seq[1,]
abund <- read.csv("ASV_abundance_Table_ITS.csv",sep=',',header = TRUE)
dim(seqs)
...
library(readr)
tax <- read_delim("ASV_tax_assignments_ITS.csv", 
                                      delim = ";", escape_double = FALSE, trim_ws = TRUE)
tax_df <- as.data.frame(tax)
library(tidyr)
dim(tax)
tax_num<-tax_df[,-1]
rownames(tax_num) <- tax_df[, 1]
dim(tax_num)
tax_num[1461:1466, 1:7]
head(tax_num)
...
library(dplyr)
sample<-factor(c("Bulk Soil","Soil surrounding root","Bulk Soil","Soil surrounding root","Bulk Soil","Soil surrounding root"))
contamination<-factor(c("High Cont","High Cont","Mid Cont","Mid Cont","Uncont","Uncont"))
sampxcont<-factor(c("BS HC","SSR HC","BS MC","SSR MC","BS UC","SSR UC"))
...
filterPhyla="k__Fungi"
data_phylo = subset_taxa(data_phylo0, Kingdom %in% filterPhyla) #para excluir !Kingdom
data_phylo
#phyloseq file ready
...
dim(data_otu_filt)
dim(abund)
1-(615/2521) # So I removed 75.6 % of the ASVs
sum(data_otu_filt)
sum(abund)
((sum(abund)-sum(data_otu_filt))/sum(abund))*100 # But only 27% of the total reads. 
...
sum_seq_rar #10625 reads

data_phylo_filt_rar_gen_f <- tax_glom(data_phylo_filt_rar, taxrank = "Genus")
data_otu_filt_rar_gen_f <- data.frame(otu_table(data_phylo_filt_rar_gen_f))

# # Access the taxonomic table and get the Genus column
genus_values <- tax_table(data_phylo_filt_rar_gen_f)[, "Genus"]
family_values <- tax_table(data_phylo_filt_rar_gen_f)[, "Family"]
order_values <- tax_table(data_phylo_filt_rar_gen_f)[, "Order"]
class_values <- tax_table(data_phylo_filt_rar_gen_f)[, "Class"]
phylum_values <- tax_table(data_phylo_filt_rar_gen_f)[, "Phylum"]
fungi_values <- tax_table(data_phylo_filt_rar_gen_f)[, "Kingdom"]
genus_values

# Replace 'g__NA' in 'genus' with corresponding 'family' values
genus_values[genus_values == "g__"] <- family_values[genus_values == "g__"]
genus_values[genus_values == "f__"] <- order_values[genus_values == "f__"]
genus_values[genus_values == "o__"] <- class_values[genus_values == "o__"]
genus_values[genus_values == "c__"] <- phylum_values[genus_values == "c__"]
genus_values[genus_values == "p__"] <- fungi_values[genus_values == "p__"]

# Replace 'g__unidentified' in 'genus' with corresponding 'family' values
...
genus_values[genus_values == "p__unidentified"] <- fungi_values[genus_values == "p__unidentified"]

# Assign the updated 'genus' column back to the tax_table in the phyloseq object
tax_table(data_phylo_filt_rar_gen_f)[, "Genus"] <- genus_values
tax_table <- data.frame(tax_table(data_phylo_filt_rar_gen_f))
...
genus
asc_positions <- which(genus == "p__Ascomycota")
chaeto_positions<- which(genus == "o__Chaetothyriales")
pleo_positions <- which(genus == "o__Pleosporales")
sorda_positions<- which(genus == "o__Sordariales")
chae_positions <- which(genus == "f__Chaetomiaceae")
helo_positions<- which(genus == "o__Helotiales")
conio_positions<- which(genus == "f__Coniochaetaceae")
cerato_positions <- which(genus == "f__Ceratobasidiaceae")
fung_positions<- which(genus == "k__Fungi")

capno_positions <- which(genus == "o__Capnodiales")
herpo_positions<- which(genus == "f__Herpotrichiellaceae")
hel_positions<- which(genus == "f__Helotiaceae")
lasio_positions <- which(genus == "f__Lasiosphaeriaceae")
dia_positions<- which(genus == "o__Diaporthales")
orbilio_positions <- which(genus == "c__Orbiliomycetes")
acre_positions<- which(genus == "g__Acremonium")
on_positions<- which(genus == "o__Onygenales")
agar_positions <- which(genus == "c__Agaricomycetes")
euro_positions<- which(genus == "o__Eurotiales")
ventu_positions <- which(genus == "o__Venturiales")
sordario_positions<- which(genus == "c__Sordariomycetes")
trem_positions<- which(genus == "o__Tremellales")
doth_positions <- which(genus == "c__Dothideomycetes")
phaeo_positions<- which(genus == "f__Phaeosphaeriaceae")
spor_positions<- which(genus == "f__Sporormiaceae")
conc_positions<- which(genus == "o__Coniochaetales")
hypo_positions <- which(genus == "o__Hypocreales")
cordyc_positions<- which(genus == "f__Cordycipitaceae")
ruts_positions<- which(genus == "f__Rutstroemiaceae")

# Detect all occurrences of duplicated values
all_duplicates <- duplicated(genus, fromLast = TRUE)
print(all_duplicates)
genus
#genus[na_positions] <- paste0("g__NA_", seq_along(na_positions))
genus[asc_positions] <- paste0("p__Ascomycota_", seq_along(asc_positions))
genus[chaeto_positions] <- paste0("o__Chaetothyriales_", seq_along(chaeto_positions))
genus[pleo_positions] <- paste0("o__Pleosporales_", seq_along(pleo_positions))
genus[sorda_positions] <- paste0("o__Sordariales_", seq_along(sorda_positions))
genus[chae_positions] <- paste0("f__Chaetomiaceae_", seq_along(chae_positions))
genus[helo_positions] <- paste0("o__Helotiales_", seq_along(helo_positions))
genus[conio_positions] <- paste0("f__Coniochaetaceae_", seq_along(conio_positions))
genus[cerato_positions] <- paste0("f__Ceratobasidiaceae_", seq_along(cerato_positions))
genus[fung_positions] <- paste0("k__Fungi_", seq_along(fung_positions))
genus[capno_positions] <- paste0("o__Capnodiales_", seq_along(capno_positions))
genus[herpo_positions] <- paste0("f__Herpotrichiellaceae_", seq_along(herpo_positions))
genus[hel_positions] <- paste0("f__Helotiaceae_", seq_along(hel_positions))
genus[lasio_positions] <- paste0("f__Lasiosphaeriaceae_", seq_along(lasio_positions))
genus[dia_positions] <- paste0("o__Diaporthales_", seq_along(dia_positions))
genus[orbilio_positions] <- paste0("c__Orbiliomycetes_", seq_along(orbilio_positions))
genus[acre_positions] <- paste0("g__Acremonium_", seq_along(acre_positions))
genus[on_positions] <- paste0("o__Onygenales_", seq_along(on_positions))
genus[agar_positions] <- paste0("c__Agaricomycetes_", seq_along(agar_positions))
genus[euro_positions] <- paste0("o__Eurotiales_", seq_along(euro_positions))
genus[ventu_positions] <- paste0("o__Venturiales_", seq_along(ventu_positions))
genus[sordario_positions] <- paste0("c__Sordariomycetes_", seq_along(sordario_positions))
genus[trem_positions] <- paste0("o__Tremellales_", seq_along(trem_positions))
genus[doth_positions] <- paste0("c__Dothideomycetes_", seq_along(doth_positions))
genus[phaeo_positions] <- paste0("f__Phaeosphaeriaceae_", seq_along(phaeo_positions))

genus[spor_positions] <- paste0("f__Sporormiaceae_", seq_along(spor_positions))
genus[conc_positions] <- paste0("o__Coniochaetales_", seq_along(conc_positions))
genus[hypo_positions] <- paste0("o__Hypocreales_", seq_along(hypo_positions))
genus[cordyc_positions] <- paste0("f__Cordycipitaceae_", seq_along(cordyc_positions))
genus[ruts_positions] <- paste0("f__Rutstroemiaceae_", seq_along(ruts_positions))
...
dim(tax_table_filt_rar_gen)
#tax_table at the genus level ready

dim(data_otu_filt_rar_gen_f)
colnames(data_otu_filt_rar_gen_f)<-genus
data_otu_filt_rar_gen_f

OTU_g = otu_table(as.matrix(data_otu_filt_rar_gen_f), taxa_are_rows = FALSE) # create the occurrence table object in phyloseq format
...
data_phylo_filt_rar
data_phylo_filt_rar_fam_f <- tax_glom(data_phylo_filt_rar, taxrank = "Family")
# # Access the taxonomic table and get the Family column
Family_values <- tax_table(data_phylo_filt_rar_fam_f)[, "Family"]
order_values <- tax_table(data_phylo_filt_rar_fam_f)[, "Order"]
class_values <- tax_table(data_phylo_filt_rar_fam_f)[, "Class"]
phylum_values <- tax_table(data_phylo_filt_rar_fam_f)[, "Phylum"]
bacteria_values <- tax_table(data_phylo_filt_rar_fam_f)[, "Kingdom"]
Family_values
# Replace 'g__NA' in 'Family' with corresponding 'family' values
Family_values[Family_values == "f__"] <- order_values[Family_values == "f__"]
Family_values[Family_values == "o__"] <- class_values[Family_values == "o__"]
Family_values[Family_values == "c__"] <- phylum_values[Family_values == "c__"]
Family_values[Family_values == "p__"] <- bacteria_values[Family_values == "p__"]

Family_values[Family_values == "f__unidentified"] <- order_values[Family_values == "f__unidentified"]
Family_values[Family_values == "o__unidentified"] <- class_values[Family_values == "o__unidentified"]
Family_values[Family_values == "c__unidentified"] <- phylum_values[Family_values == "c__unidentified"]
Family_values[Family_values == "p__unidentified"] <- bacteria_values[Family_values == "p__unidentified"]
Family_values

# Assign the updated 'Family' column back to the tax_table in the phyloseq object
tax_table(data_phylo_filt_rar_fam_f)[, "Family"] <- Family_values
tax_table <- data.frame(tax_table(data_phylo_filt_rar_fam_f))

# Check the tax_table to verify changes
tax_table
tax_table_filt_rar_fam_f <- tax_table[,-c(6:7)]
tax_table_filt_rar_fam_f

Family <- as.character(tax_table_filt_rar_fam_f$Family)
Family

# Make unique by appending .1, .2, etc.
Family_unique <- make.unique(Family, sep = "_")
Family_unique

row.names(tax_table_filt_rar_fam_f)<-Family_unique

data_phylo_filt_rar_fam_f
tax_fam_f <- as.data.frame(tax_table(data_phylo_filt_rar_fam_f))
row.names(tax_fam_f) <- Family_unique
dim(tax_fam_f)
tax_table_filt_rar_fam_f

data_otu_filt_rar_fam_f = data.frame(otu_table(data_phylo_filt_rar_fam_f)) 
data_otu_filt_rar_fam_f
tax_fam_f
colnames(data_otu_filt_rar_fam_f)<-rownames(tax_fam_f)
data_otu_filt_rar_fam_f

#new phyloseq
OTU = otu_table(as.matrix(data_otu_filt_rar_fam_f), taxa_are_rows = FALSE) # create the occurrence table object in phyloseq format
SAM = sample_data(mapping, errorIfNULL = TRUE) # create the sample metadata object in phyloseq format
TAX = tax_table(as.matrix(tax_table_filt_rar_fam_f)) # create the observation metadata object (OTU taxonomy) in phyloseq format
phylo_fun_f <- phyloseq(OTU, TAX, SAM)
phylo_fun_f


#Filter by category
Uncont_fam_f_RA <- data_otu_filt_rar_fam_f[grepl("Uncont", rownames(data_otu_filt_rar_fam_f)), ]
SC_fam_f_RA <- data_otu_filt_rar_fam_f[grepl("SC", rownames(data_otu_filt_rar_fam_f)), ]
Cont_fam_f_RA <- data_otu_filt_rar_fam_f[grepl("Cont", rownames(data_otu_filt_rar_fam_f)), ]
BS_fam_f_RA <- data_otu_filt_rar_fam_f[!grepl("BS", rownames(data_otu_filt_rar_fam_f)), ]
SSR_fam_f_RA <- data_otu_filt_rar_fam_f[grepl("SSR", rownames(data_otu_filt_rar_fam_f)), ]
SSR_fam_f_RA
...
str(tab_diver2)
library(readxl)
BGQ2 <- read_excel("BGQ3.xlsx", sheet = "All")
dim(tab_diver2)
dim(BGQ)
...
BS<-data_otu_filt_rar[c(1:10,21:30,41:50),]
BS_map<-mapping[c(1:10,21:30,41:50),]
BS_map
SSR<-data_otu_filt_rar[c(11:20,31:40,51:60),]
SSR_map<-mapping[c(11:20,31:40,51:60),]

HC<-data_otu_filt_rar[c(1:20),]
HC_map<-mapping[c(1:20),]
MC<-data_otu_filt_rar[c(21:40),]
MC_map<-mapping[c(21:40),]
UC<-data_otu_filt_rar[c(41:60),]
UC_map<-mapping[c(41:60),]
UC_map
...
data_phylo_filt_rar_fam_f
tax_data = data.frame(tax_table(data_phylo_filt_rar_fam_f)) # create a separated file
otu_data = (data.frame(otu_table(data_phylo_filt_rar_fam_f)))
sam_data = data.frame(sample_data(data_phylo_filt_rar_fam_f))
...
BS_mt$otu_table <- BS_mt$otu_table[,c(1:10,21:30,41:50)]
BS_mt$tax_table
BS_mt$otu_table
BS_mt$tidy_dataset()
BS_mt
...
# not printed at 1600x800 rel_abund
...
t1$for_what<- 'fungi'
t1$cal_spe_func(fungi_database = list(fungi_func_FungalTraits))
...
df_long <- df_joined %>%
  pivot_longer(
    cols = starts_with("primary_lifestyle"),
    names_to = "Function",
...
df_long <- df_long %>%
  mutate(
    Group = case_when(
      str_detect(Function, "parasite") ~ "Parasites",
      str_detect(Function, "mycorrhizal") ~ "Mycorrhizas",
      str_detect(Function, "symbiont") ~ "Other symbionts",
      str_detect(Function, "saprotroph") ~ "Saprotrophs",
      TRUE ~ "Other"
    )
  )

df_sum <- df_long %>%
  group_by(Sample,contamination,sample,sampxcont,Group) %>%
  summarise(
    sum_perc = sum(Percentage, na.rm = TRUE)
  ) %>%
  ungroup()
df_sum

abund_summary <- df_sum %>%
  group_by(Group, sample, contamination, sampxcont) %>%
  summarise(
    mean_abund = mean(sum_perc, na.rm = TRUE),
    se_abund = sd(sum_perc, na.rm = TRUE) / sqrt(n())
  ) %>%
  ungroup()
print(abund_summary)
...
for (cont in cont_levels) {
  ...
  p <- ggplot(df_plot, aes(x = Group, y = mean_abund, fill = sample)) +
    geom_bar(stat = "identity", position = position_dodge(width = 0.9), color = "black") +
    geom_errorbar(aes(ymin = mean_abund - se_abund, ymax = mean_abund + se_abund),
                  position = position_dodge(width = 0.9), width = 0.2, color = "black") +
    scale_fill_manual(values = custom_colors) +
...
unique_cont <- unique(df_sum$contamination)
results_list <- list()

for (c in unique_cont) {
  df_c <- df_sum %>% filter(contamination == c)
  
  unique_groups <- unique(df_c$Group)
  ...
  for (g in unique_groups) {
       df_gc <- df_c %>% filter(Group == g)
    
    if (length(unique(df_gc$sample)) == 2) {
    
        test_result <- wilcox.test(sum_perc ~ sample, data = df_gc)
      p_val <- test_result$p.value
      
      results_c <- rbind(
        results_c,
        data.frame(Function = g, p_value = p_val)
      )
      
    } else {
      results_c <- rbind(
        results_c,
        data.frame(Function = g, p_value = NA)
      )
    }
  }
  print(results_c)
...
results_list[["High Cont"]]
results_list[["Mid Cont"]]
results_list[["Uncont"]]
